...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Timeout promise after 10s to prevent getting stuck
      ...
      if (
        ...
        setError("Email atau kata sandi tidak cocok. Jika baru pertama kali, silakan buat akun di menu 'Daftar Sekarang'.");
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === "auth/operation-not-allowed") {
        ...
        setError("Jendela login Google ditutup sebelum selesai.");
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      <div className="w-full max-w-md p-6 sm:p-8 rounded-3xl bg-surface border border-border shadow-float space-y-6">
        ...
        <div className="text-center text-xs text-muted pt-2 border-t border-border/50">
          Belum punya akun?{" "}
          <Link href="/register" className="font-bold text-[#7C5CFA] hover:underline">
